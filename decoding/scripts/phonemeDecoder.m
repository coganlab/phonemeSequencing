...
%% Phoneme results
load([subjectId '_sigChannel.mat'])
decoderObj = decoderClass(20,[10:10:90]);
decodeResultStruct = decoderObj.baseClassify(ieegHGRespNorm,phonemeTrial.phonemeUnit(:,1)',[-0.5 0.5], sigChannel)

syllableUnit = phonemeTrial.syllableUnit(:,1)';
phonemeUnits = phonemeTrial.phonemeUnit(syllableUnit==1,1)';
ieegHGVowel = ieegHGRespNorm;
ieegHGVowel.data = ieegHGVowel.data(:,syllableUnit==1,:);
decodeResultStruct = decoderObj.baseClassify(ieegHGVowel,phonemeUnits,[-0.5 0.5], sigChannel)
