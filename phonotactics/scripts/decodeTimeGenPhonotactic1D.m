function  decodeTimeGenPhonotactic1D(matfilename,numCore)
...
for iPhon = 1:length(phontactid)
    ...
    phontactfeature = (phonemeTrialPooled.phonotactic(cvcIds,phontactid(iPhon))');
    ...
    decodeTimeStruct1D_cvc{iPhon} = phonDecode.tempGenClassify1D(ieegStruct,...
            phontactcat,timeRes = timeRes1D, timeWin = timeWin);
    save([pwd '/workspace/results/' matfilename(1:length(matfilename)-4) '_Start_phonotactic_cvc_decoded_1D.mat'],...   
     'decodeTimeStruct1D_cvc','channelNamePooled','-v7.3');
end
