...
labelVals{1}='precentral';
labelVals{2}='postcentral';
labelVals{3}='superiortemporal';
labelVals{4}='supram';
labelVals{5}='Triang';
labelVals{6}='Operc';
labelVals{7}='parietal';
...
for iL=1:length(labelVals)
    ...
    load([TASK_DIR '/forKumar/prodDelayElecs.mat']);
    ...
    SNList=unique(labelChan(sigDelayIdx,1));
...
%     SNList=SNList2;
    ...
    for iC=1:length(Task.Conds)
        for iF=1:length(Task.Conds(iC).Field)
            condMat=zeros(length(SNList),126); % 126 specific triggers, but zeros in the middle
            for iSN=1:length(SNList)
