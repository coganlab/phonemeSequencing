...
%% HG power extraction & Phoneme decoding
    ...
    %                         acc = trace(CmatAll)/sum(sum(CmatAll));        
            ...
            disp('done');        
          save(strcat(dLabels(iSubject).name,'_phonDecodeMotorHGPackSigChannelCarAnatSignificant_v3.mat'),'accChan','allChannels','CMatCat','anatChan','specPower','pvalsMCleanProd','accChan','accPhoneme','accPhonemeUnbias','accPhonemeChance','phonError','ieegGammaSig','phonIndClass','timeRange','accTime');
    end
close all;
end